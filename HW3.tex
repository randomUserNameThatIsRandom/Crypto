...
\section{•}
As hinted, we set $Enc_{pk}(r, b) = (F_pk(r) , B(r) \oplus b)$, and set $Dec_{sk}(r', b')=Inv_{sk}(r'),B(Inv_{sk}(r')) \oplus b'$.
\textbf{Correctness}:
\begin{align*}
Dec_{sk}(Enc_{pk}(r,b)) & =Dec_{sk}(F_{pk}(r),B(r)\oplus b)\\
 & =(Inv_{sk}(F_{pk}(r)),B(Inv_{sk}(F_{pk}(r)))\oplus B(r)\oplus b)\\
 & =\left(r,B(r)\oplus B(r)\oplus b\right)\\
 & =\left(r,0\oplus b\right)\\
 & =\left(r,b\right)
\end{align*}
\textbf{Semantically-Secure}:
